...
// Badge yacht CSS class -> badge car/general
content = content.replace(
  ".badge-yacht { background: rgba(16, 185, 129, 0.15); color: #34d399; border: 1px solid rgba(16, 185, 129, 0.35); }",
  ".badge-yacht { background: rgba(245, 158, 11, 0.15); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.35); }"
);
...
const currDesktopOld = `<select id="currency-select" class="input-field" style="padding: 0.4rem 0.8rem; font-size: 0.8rem; width: auto; background: rgba(255,255,255,0.05); border-radius: var(--radius-sm);" onchange="updateCurrency(this.value)">
...
const currDesktopNew = `<select id="currency-select" class="input-field" style="padding: 0.4rem 0.8rem; font-size: 0.8rem; width: auto; background: rgba(255,255,255,0.05); border-radius: var(--radius-sm);" onchange="updateCurrency(this.value)">
...
content = content.replace(currDesktopOld, currDesktopNew);

const currMobileOld = `<select id="mobile-currency-select" class="input-field" style="padding: 0.4rem 0.8rem; font-size: 0.82rem; width: auto; background: rgba(255,255,255,0.06); border-radius: var(--radius-sm);" onchange="updateCurrency(this.value)">
            <option value="USD">USD ($)</option>
            <option value="EUR">EUR (€)</option>
            <option value="GBP">GBP (£)</option>
            <option value="AED">AED (د.إ)</option>
...
const currMobileNew = `<select id="mobile-currency-select" class="input-field" style="padding: 0.4rem 0.8rem; font-size: 0.82rem; width: auto; background: rgba(255,255,255,0.06); border-radius: var(--radius-sm);" onchange="updateCurrency(this.value)">
            <option value="GHS" selected>GHS (GH₵)</option>
            <option value="USD">USD ($)</option>
            <option value="EUR">EUR (€)</option>
            <option value="GBP">GBP (£)</option>
...
content = content.replace(currMobileOld, currMobileNew);

// 6. Hero header text
const heroTitleOld = `<div style="max-width: 900px; margin-bottom: 2.5rem; z-index: 10;">
...
const heroTitleNew = `<div style="max-width: 920px; margin-bottom: 2.5rem; z-index: 10;">
        <div style="display: inline-flex; align-items: center; gap: 0.6rem; background: rgba(255,255,255,0.06); border: 1px solid var(--border-gold); padding: 0.55rem 1.4rem; border-radius: 40px; margin-bottom: 2rem; box-shadow: var(--shadow-gold);">
          <span style="color: var(--accent-gold); font-size: 1.1rem;">✦</span>
...
// Search placeholder & quick destination pills
content = content.replace(
...
// 8. Add Ghana's Most Wanted Fleet Showcase right after KPI banner
...
// 11. Partner registration category options
...
content = content.replace('placeholder="E.g. 14 Supercars & 4 Yachts"', 'placeholder="E.g. 18 Luxury SUVs (BMW, GLE, V8) & 12 City Cruisers (Vitz/Yaris)"');
...
content = content.replace('placeholder="E.g. Rossi Luxury Marine SARL"', 'placeholder="E.g. Gold Coast Executive Motors & Stays Ltd."');

// 12. Review modal in HTML
content = content.replace('id="rev-applicant-name">Marco Rossi</h2>', 'id="rev-applicant-name">Akosua Mensah</h2>');
content = content.replace('id="rev-category-badge">Supercars & Yachts</span>', 'id="rev-category-badge">Executive Fleet</span>');
content = content.replace('id="rev-company-title">Rossi Supercars & Riva Yachting Group Ltd.</p>', 'id="rev-company-title">Gold Coast Prestige Motors & Chauffeur Services Ltd.</p>');
content = content.replace('id="rev-submission-meta">Submitted: Sep 21, 2026 • Port Hercule, Monaco 🇲🇨</p>', 'id="rev-submission-meta">Submitted: Sep 21, 2026 • Airport City, Accra 🇬🇭</p>');
content = content.replace('id="rev-kpi-hq">Monaco 🇲🇨</h4>', 'id="rev-kpi-hq">Accra 🇬🇭</h4>');
content = content.replace('id="rev-kpi-scale">14 Supercars & 4 Yachts</h4>', 'id="rev-kpi-scale">18 SUVs & 12 Cruisers</h4>');
content = content.replace('id="rev-legal-name">Rossi Supercars & Riva Yachting Group Ltd.</strong>', 'id="rev-legal-name">Gold Coast Prestige Motors & Chauffeur Services Ltd.</strong>');
content = content.replace('id="rev-reg-num">MC-984420-VAT</strong>', 'id="rev-reg-num">GH-CS-892401</strong>');
content = content.replace('id="rev-tax-id">MC-TX-88219</strong>', 'id="rev-tax-id">GH-TIN-P0029104</strong>');
content = content.replace('id="rev-rep-name">Marco Rossi (Managing Director)</strong>', 'id="rev-rep-name">Akosua Mensah (Managing Director)</strong>');
content = content.replace('id="rev-rep-web">https://rossi-marine.mc</a>', 'id="rev-rep-web">https://goldcoast-motors.com.gh</a>');

// 13. Add asset modal options
const newAssetOld = `<select id="new-asset-type" class="input-field">
...
const newAssetNew = `<select id="new-asset-type" class="input-field">
                    <option value="HOTEL">Hotel Suite / Luxury Villa</option>
...
content = content.replace(newAssetOld, newAssetNew);

// 14. Testimonials & Footer
...
// 15. REPLACE DEFAULT_SEED_DATA USING EXACT DELIMITERS
const startSeedDelim = 'const DEFAULT_SEED_DATA = {';
const endSeedDelim = '    // Load from localStorage or set default';

const startIndex = content.indexOf(startSeedDelim);
const endIndex = content.indexOf(endSeedDelim);

if (startIndex === -1 || endIndex === -1) {
  throw new Error(`Could not find seed delimiters: ${startIndex}, ${endIndex}`);
}

const newGhanaSeed = `const DEFAULT_SEED_DATA = {
      currentPage: 'home',
      ...
      users: [
        ...
        },
        {
          ...
          application: {
            companyName: 'Gold Coast Prestige Motors & Chauffeur Services Ltd.',
      ...
      listings: [
        {
          ...
          description: "Accra's premier 5-star destination in the downtown diplomatic enclave. Unparalleled luxury suites, 25-meter infinity pool, Resense Spa, and VIP executive lounge access.",
          serviceType: 'HOTEL',
          status: 'APPROVED',
...
content = content.slice(0, startIndex) + newGhanaSeed + content.slice(endIndex);

// 16. Update loadInitialState, saveState, resetDemoData
...
const loadInitialNew = `function loadInitialState() {
      ...
      // Wipe stale keys from earlier builds
      localStorage.removeItem('aether_booking_state_v3');
...
// 17. Clean up JS functions that reference YACHT
...
content = content.replace("${listing.serviceType === 'HOTEL' || listing.serviceType === 'YACHT' ?", "${listing.serviceType === 'HOTEL' ?");
...
// JS Partner review modal defaults
content = content.replace(
...
content = content.replace(
  "const cat = document.getElementById('auth-partner-category').value || 'Supercars, Yachts & Private Drivers';",
  "const cat = document.getElementById('auth-partner-category').value || 'Executive Cars & Chauffeur Services';"
);

// Save updated index.html
fs.writeFileSync(indexPath, content, 'utf8');
console.log('Saved index.html successfully. Total lines:', content.split('\n').length);

// Sync to demo.html
const demoPath = path.join(__dirname, '..', 'demo.html');
fs.writeFileSync(demoPath, content, 'utf8');
console.log('Synced demo.html successfully.');
